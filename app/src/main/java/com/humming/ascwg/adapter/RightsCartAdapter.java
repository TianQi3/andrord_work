...
public class RightsCartAdapter extends AbstractItemPagerArrayAdapter<OrderSelect, RightsViewHolder> {
    private View.OnClickListener onbtnDeleteClickListener;
    private View.OnClickListener onbtnItemClickListener;
    private String types;
    private Context context;
    public List<OrderSelect> rightSelects;
    private int radio = 1;

    public RightsCartAdapter(final Context context, int resource, final List<OrderSelect> items, int[] itemPageResArray, int defaultPageIndex, String types) {
        super(context, resource, items, itemPageResArray, defaultPageIndex, types);
        this.types = types;
        this.context = context;
        this.rightSelects = items;
            ...
            public void onClick(View v) {
                ...
                OkHttpClientManager.postAsyn(Config.SHOPPINGCART_DELETE, new OkHttpClientManager.ResultCallback<ResponseData>() {
                    ...
                    public void onResponse(ResponseData response) {
                        ...
                        if (items.size() == 0) {
                            ShoppingCartContent.yumAll.setVisibility(View.GONE);
                        }
                        ShoppingCartContent.measureListViewHeight(ShoppingCartContent.mRecycleViewYum);
    ...
    @Override
    protected RightsViewHolder createViewHolder() {
        return new RightsViewHolder();
    }

    @Override
    protected void initViewHolder(int position, RightsViewHolder viewHolder, List<View> itemPages) {
        ...
        viewHolder.sendSize = (TextView) centerView.findViewById(R.id.item_rights_yum__send_size);
    ...
    @Override
    protected void setItemData(final int position, final RightsViewHolder viewHolder, final OrderSelect itemData) {
        if (rightSelects.get(position).isSelect()) {
        ...
        viewHolder.mCartsub.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (number == 1) {
                    ...
                    number--;
                    rightSelects.get(position).setQuantity(rightSelects.get(position).getQuantity() - 1);
                    viewHolder.mCartNumber.setText(number + "");
                    int sendSize = number / 3;
                    viewHolder.sendSize.setText("×" + sendSize);
                    if (viewHolder.mCartItemRadio.isChecked() && rightSelects.get(position).isSelect()) {
        ...
        viewHolder.mCartItemRadio.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    rightSelects.get(position).setSelect(true);
                    ShoppingCartContent.rightsLists.get(position).setSelect(true);
                    boolean b = false;
                    for (OrderSelect orderSelect : rightSelects) {
                        if (!orderSelect.isSelect()) {
                            b = true;
                        }
                    }
                    if (!b) {
                        ShoppingCartContent.someOrAllSelect = false;
                        ShoppingCartContent.selectYum.setChecked(true);
                    ...
                    rightSelects.get(position).setSelect(false);
                    ShoppingCartContent.rightsLists.get(position).setSelect(false);
                    ...
                    if (ShoppingCartContent.selectYum.isChecked()) {
                        ShoppingCartContent.someOrAllSelect = false;
                        ShoppingCartContent.selectYum.setChecked(false);
                    }
